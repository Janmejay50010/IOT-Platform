...
def add_user_algo_instance(email,base_algo,algo_instance,run_type,sensor_dict,action_dict,schedule=None,interval=0):
    if run_type=="now":
        print(interval)
        db.users.insert_one({"email":email,"AlgorithmName":base_algo,"AlgorithmID":algo_instance,"run_type":run_type,'sensor':sensor_dict,'action':action_dict,'interval':interval})
    else:
...
def get_algo_status(user,algoID):
    local_db=client['platformDeployer']
    status=local_db.ContainerStatus.find_one({"UserName":user,"ApplicationId":algoID})
    if status is not None and 'Status' in status and status['Status']=='running':
        return "on"
    return "off"
